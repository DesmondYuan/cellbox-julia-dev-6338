...
"""
<----------------------------------------------------------------------------->
Grid EXPR_B2: Quality tests
~100s * 7 seeds = 12min
Part I: Gaussian Noise: σ ∈ [0, 0.1, 0.2, 0.5, 1.0, 3.0]
    6σ x 2n x 2m = 24 => 5 hours

Note that L2_LAMBDA = 1e-4 is updated.
<----------------------------------------------------------------------------->
"""
...
# Main
for noise_level in [0, 0.1, 0.2, 0.5, 1.0, 3.0]
    ...
    for n in [32, 256]
        ...
        for m in [10, 20]
            ...
            EXPR_NAME = "Gaussian_DAG$(m)_n$(n)_sigma$(noise_level)"
            NUM_NODES = m
